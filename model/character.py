...
class LayeredBody:
    def __init__(self, surface, x=250, y=100, color=BLACK):
...
class MaleBody(LayeredBody):
    def draw_head(self, surface):
        pygame.draw.circle(surface, self.color, (self.x, self.y), 44, 2)  # bigger head

    def draw_neck(self, surface):
        pygame.draw.rect(surface, self.color, (self.x - 12, self.y + 44, 24, 20), 2)  # thicker neck

    def draw_chest(self, surface):
        pygame.draw.rect(surface, self.color, (self.x - 35, self.y + 64, 70, 65), 2)  # wider chest

    def draw_stomach(self, surface):
        pygame.draw.rect(surface, self.color, (self.x - 18, self.y + 129, 36, 45), 2)  # taller stomach
